...
import type { Task, Skill } from "../TaskManager/TaskManager";
import type { Person } from "../PeopleTable/PeopleTable";
import type { RespProject } from "../ProjectCards/ProjectCard";
import "./TaskForm.css";
...
export const TaskForm = ({
	...
	const [selectedRequiredTasks, setSelectedRequiredTasks] = useState<
		MiniTask[]
	>([]);
	...
	useEffect(() => {
		const fetchSkills = async () => {
			...
			people.map((person: Person) =>
				person.skills.map((skill) => skillSet.add(skill.name)),
			);
		...
		const fetchTasks = async () => {
			...
			const tasksToPick = project.tasks.filter(
	...
	const taskOptions = requiredTasks
		.filter((task) =>
			task.name.toLowerCase().includes(taskSearch.trim().toLowerCase()),
		)
		.map((task) => (
			<Combobox.Option
				value={task.name}
			...
			>
				<Group gap="sm">
					{selectedRequiredTasks.some((t) => t.id === task.id) ? (
						<CheckIcon size={12} />
					) : null}
					<span>{task.name}</span>
				</Group>
